...
class BoardState():
    ...
    def getHVal(self, heuristic_type: str) -> int:
        if heuristic_type == "Sliding":
            ...
            return self.calc_total_euclidean_for_board(self.board)
                ...
                if self.board_array[row][col] == 0:
                    ...
                    if delta_y >= 0 and delta_y < self.side_length:
                        if self.board_array[delta_y][col] != 0:
                            ...
                            states.append(BoardState(current_copy, self.goal_array,
                                                     self.heuristic_type, self.weighted, self.blanks, self.manhattan_h_val, self.euclidean_h_val, self.displaced_tiles, self.model, self.scaler, g+1, self, move_title=f"Move {self.board_array[delta_y][col]} up", effort=self.board_array[delta_y][col], node_depth=node_depth+1))
                    # Down
                    delta_y = row-1
                    if delta_y >= 0 and delta_y < self.side_length:
                        if self.board_array[delta_y][col] != 0:
                            ...
                            current_copy[delta_y][col] = 0
                            states.append(BoardState(current_copy, self.goal_array,
                                                     self.heuristic_type, self.weighted, self.blanks, self.manhattan_h_val, self.euclidean_h_val, self.displaced_tiles, self.model, self.scaler, g+1, self, move_title=f"Move {self.board_array[delta_y][col]} down", effort=self.board_array[delta_y][col], node_depth=node_depth+1))
                    # Left
                    delta_x = col-1
                    if delta_x >= 0 and delta_x < self.side_length:
                        if self.board_array[row][delta_x] != 0:
                            ...
                            states.append(BoardState(current_copy, self.goal_array,
                                                     self.heuristic_type, self.weighted, self.blanks, self.manhattan_h_val, self.euclidean_h_val, self.displaced_tiles, self.model, self.scaler, g+1, self, move_title=f"Move {self.board_array[row][delta_x]} left", effort=self.board_array[row][delta_x], node_depth=node_depth+1))
                    # Right
                    delta_x = col+1
                    if delta_x >= 0 and delta_x < self.side_length:
                        if self.board_array[row][delta_x] != 0:
                            ...
                            current_copy[row][delta_x] = 0
                            states.append(BoardState(current_copy, self.goal_array,
                                                     self.heuristic_type, self.weighted, self.blanks, self.manhattan_h_val, self.euclidean_h_val, self.displaced_tiles, self.model, self.scaler, g+1, self, move_title=f"Move {self.board_array[row][delta_x]} right", effort=self.board_array[row][delta_x], node_depth=node_depth+1))
        return states
